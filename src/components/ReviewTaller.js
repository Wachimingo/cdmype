...
class ReviewTaller extends Component {
  ...
  review(e){
    ...
    e.preventDefault();
    this.setState({hecho: true});
    localStorage.setItem('idreviewactivo'+contador, this.props.idreviewactivo);
    contador++;
    ...
    $.ajax({
            ...
            contentType: false,
            processData: false,
            success: () => this.incrementar(contador)
  ...
  renderMensaje(hecho){
    if (!hecho) {
      ...
      return(
      <div className="container-fluid ">
        <h5>Sus respuesta sera procesada en unos minutos</h5>
